...
export function GifActivity() {
  ...
  const fetchNewQuestion = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/activity2/gifs/question`);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-cyan-100 to-white flex items-center justify-center">
        <div className="animate-bounce text-3xl mb-4">🎭</div>
        <p className="font-comic text-gray-600">Loading your GIF...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-100 to-white p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="font-comic font-bold text-gray-700">
              GIF {currentIndex + 1} of {Math.max(history.length, currentIndex + 1)}
            </span>
            <button
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-xl p-6">
          <h2 className="text-2xl font-baloo font-bold text-gray-800 mb-6 text-center">
            What feeling does this GIF express?
          </h2>
            ...
            <div className={`text-center p-4 rounded-lg mb-6 ${
              ...
              {isCorrect ? (
                <p className="font-comic font-bold">
                  🎉 Correct! You're great at reading emotions! 🌟
                </p>
              ) : (
                <p className="font-comic font-bold">
                  The correct emotion was {
